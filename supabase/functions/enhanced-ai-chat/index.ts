...
## 🎯 MULTI-CHART VISUALIZATION - PATTERN B (MANDATORY):

**Generate 2 Complementary Charts When Data Supports It:**

When analyzing structured data with multiple dimensions, ALWAYS generate an array of visualData with:
1. **Metric Cards** (2-4 key KPIs)
2. **Primary Chart** (main insight - bar/line/pie based on data)
3. **Secondary Chart** (complementary perspective - different chart type)

**Example Multi-Chart Response for "show content performance":**
\`\`\`json
[
  {
    "type": "metrics",
    "metrics": [
      {"id": "total", "title": "Total Content", "value": 45, "icon": "FileText", "color": "blue"},
      {"id": "published", "title": "Published", "value": 32, "icon": "CheckCircle2", "color": "green"},
      {"id": "drafts", "title": "Drafts", "value": 13, "icon": "Edit", "color": "orange"},
      {"id": "seo", "title": "Avg SEO Score", "value": "75.2", "icon": "TrendingUp", "color": "purple"}
    ]
  },
  {
    "type": "chart",
    ...
    "chartConfig": {
      ...
      "data": [
        ...
        {"name": "Drafts", "value": 13}
      ],
      "categories": ["value"],
      "colors": ["hsl(var(--success))", "hsl(var(--primary))", "hsl(var(--warning))"]
    }
  },
  {
    "type": "chart",
    ...
    "chartConfig": {
      "type": "line",
      "data": [
        {"name": "Week 1", "created": 12},
        {"name": "Week 2", "created": 15},
        {"name": "Week 3", "created": 8},
        {"name": "Week 4", "created": 10}
      ],
      "series": [{"dataKey": "created", "name": "Content Created", "color": "hsl(var(--primary))"}],
      "categories": ["name"]
    }
  }
]
...
**Chart Pairing Guidelines:**
- Status/Distribution + Timeline/Trend
- Performance Bar Chart + Growth Line Chart
- Comparison Bar Chart + Distribution Pie Chart
- Volume Bar Chart + Success Rate Line Chart

**When to Use Pattern B (2 charts):**
✅ User asks about "performance", "analytics", "overview", "status"
✅ Data has both categorical AND time-series dimensions
✅ Data supports showing different perspectives (what + how/when)
✅ You want to tell a complete data story

**When to Use Single Chart:**
⚠️ Data only supports one perspective
⚠️ User asks specific question about one metric
⚠️ Limited data available (< 3 data points)
...
Provide comprehensive, data-driven responses that ALWAYS include relevant actions and visual insights. Remember: Every response should help the user take action and understand data visually.`;
    ...
    // AGGRESSIVE CHART AUTO-GENERATION - Pattern B (2 charts when data supports)
    const shouldAutoGenerateCharts = (
      chartRequest.requested || 
      /performance|analytics|metrics|status|overview|show me|how many|what's my|progress|growth|timeline|distribution/i.test(userQuery)
    );
    
    if (shouldAutoGenerateCharts && !visualData) {
      console.log('🤖 AUTO-GENERATING CHARTS: No visualData from AI, generating from real data...');
      
      // Parse real data context to generate charts
      const contentMatch = realDataContext.match(/Total Content Items: (\d+)/);
      const publishedMatch = realDataContext.match(/Published: (\d+)/);
      const draftMatch = realDataContext.match(/Draft: (\d+)/);
      const avgSeoMatch = realDataContext.match(/Average SEO Score: ([\d.]+)/);
      const proposalsMatch = realDataContext.match(/Total Proposals: (\d+)/);
      const availableMatch = realDataContext.match(/Available Opportunities: (\d+)/);
      
      if (contentMatch || proposalsMatch) {
        const visualDataArray = [];
        
        // Generate metrics card
        const metrics = [];
        if (contentMatch) metrics.push({ id: 'total-content', title: 'Total Content', value: parseInt(contentMatch[1]), icon: 'FileText', color: 'blue' });
        if (publishedMatch) metrics.push({ id: 'published', title: 'Published', value: parseInt(publishedMatch[1]), icon: 'CheckCircle2', color: 'green' });
        if (draftMatch) metrics.push({ id: 'drafts', title: 'Drafts', value: parseInt(draftMatch[1]), icon: 'Edit', color: 'orange' });
        if (avgSeoMatch) metrics.push({ id: 'avg-seo', title: 'Avg SEO Score', value: parseFloat(avgSeoMatch[1]).toFixed(1), icon: 'TrendingUp', color: 'purple' });
        
        if (metrics.length > 0) {
          visualDataArray.push({ type: 'metrics', metrics });
        }
        
        // Generate Chart 1: Status distribution (Bar chart)
        if (contentMatch && publishedMatch && draftMatch) {
          const totalContent = parseInt(contentMatch[1]);
          const published = parseInt(publishedMatch[1]);
          const draft = parseInt(draftMatch[1]);
          const inPipeline = totalContent - published - draft;
          
          visualDataArray.push({
            type: 'chart',
            title: 'Content Status Distribution',
            description: 'Breakdown of content by status',
            chartConfig: {
              type: 'bar',
              data: [
                { name: 'Published', value: published },
                { name: 'In Pipeline', value: Math.max(0, inPipeline) },
        ...
        // Generate Chart 2: Proposals/Opportunities (Pie chart if proposals exist)
        if (proposalsMatch && availableMatch) {
          const totalProposals = parseInt(proposalsMatch[1]);
          const available = parseInt(availableMatch[1]);
          const other = totalProposals - available;
          
          if (totalProposals > 0) {
            visualDataArray.push({
              type: 'chart',
              title: 'Content Opportunities',
              description: 'Available vs actioned proposals',
              chartConfig: {
                type: 'pie',
                data: [
                  { name: 'Available', value: available },
                  { name: 'Scheduled/Completed', value: Math.max(0, other) }
                ],
                categories: ['value'],
        ...
        // Set visualData as array if we have multiple charts
        if (visualDataArray.length > 0) {
          visualData = visualDataArray.length === 1 ? visualDataArray[0] : visualDataArray;
          console.log(`✅ AUTO-GENERATED ${visualDataArray.length} visualizations from real data`);
        }
      }
    }
    
    // Chart validation - Only convert existing visual data, never force generation
    if (chartRequest.requested && visualData && !Array.isArray(visualData) && visualData.type !== 'chart') {
