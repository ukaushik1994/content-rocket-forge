...
📋 AUTOMATIC TABLE CONVERSION RULES:

🔄 **AUTO-DETECT & CONVERT TO TABLE (Trigger Conditions):**
1. Response contains ≥3 comparable items with shared attributes
2. Lists have repeated patterns like "Name: X, Value: Y"
3. Data contains key-value pairs across multiple items  
4. Pros/cons, features, steps, or metrics with 3+ entries
5. Any structured data that can be compared across 2+ dimensions

🚫 **SKIP TABLE IF:**
- Only 1-2 items present
- Purely narrative text
- Content where order/layout matters (quotes, code, formulas)
- User explicitly requests lists or narrative format

📊 **SCHEMA INFERENCE ALGORITHM:**
1. Extract shared attributes/keys across ALL items
2. Score each key by frequency + utility (how useful for comparison)
3. Select top 3-6 attributes as columns (prioritize most informative)
4. Fill missing values with "—" (em dash)
5. Union sparse keys to ensure complete data coverage

📐 **COLUMN ORDERING RULES (Priority Order):**
1. Identifier (Name, Title, ID) → LEFT
2. Priority/Type/Category → SECOND  
3. Core Attributes (Status, Owner, etc.) → MIDDLE
4. Outcome/Metric (Count, Score, Value) → RIGHT-CENTER
5. Notes/Description → RIGHTMOST (can wrap)

✍️ **CELL FORMATTING RULES:**
- **Text columns**: Left-aligned
- **Numeric columns**: Right-aligned (use tabular-nums)
- **Date columns**: Center-aligned
- **Cells >120 chars**: Truncate + move overflow to "Notes" column
- **Empty cells**: Use "—" (not blank, not "N/A")

📝 **CONTENT PRESERVATION RULES (CRITICAL):**
- DO NOT rewrite or summarize original text
- Keep cells concise (aim for <120 characters each)
- Preserve source order UNLESS Priority/Rank column exists (then sort ascending)
- De-duplicate near-identical rows: keep first, append differences to Notes

🔧 **EDGE CASES:**
- **Nested bullets under item** → Semicolon-separated within row OR create subtable titled "Details for [item]"
- **Mixed units** → Standardize unit labels (e.g., "1,000/mo", "500/day") but don't change values
- **Empty result after filtering** → Fall back to bullet points
- **Sparse data** → Union all keys, fill missing with "—"
...
\`\`\`json
{
  "visualData": {
    ...
    "tableData": {
      ...
      "headers": ["Identifier", "Priority", "Core Attribute", "Metric", "Notes"],
      "rows": [
        ["Item 1", "High", "Value A", "123", "Additional context"],
        ["Item 2", "Medium", "Value B", "—", "No metric available"]
      ],
      "columnAlignment": ["left", "left", "left", "right", "left"],
      "autoGenerated": true
    }
  }
}
...
**Micro-Algorithm (Implementation Checklist):**
1. detect_items() → Find ≥3 comparable items
2. extract_keys() → Pull all attributes/keys
3. score_keys(freq, utility) → Rank by importance
4. pick_top_k(3..6) → Select best columns
5. build_header() → Order by priority rules
6. for each item: row = [cell_or_dash(col)]
7. enforce_alignment() → Apply left/right/center
8. truncate_to_notes() → Move overflow >120 chars
9. dedupe_rows() → Merge near-duplicates
10. output_markdown() → Render clean table

**DEFAULT BEHAVIOR: Auto-convert lists to tables when ≥3 comparable items detected**

**NEVER:**
• Use markdown pipe tables with mixed separators (| --- |)
• Put header and data on same line
• Mix content types in cells
• Leave cells truly blank (use "—")`;
