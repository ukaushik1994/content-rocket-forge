-- Enterprise Tables for Phase 4 (only create missing tables)
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'team_workspaces' AND policyname = 'Users can create their own workspaces') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'team_members' AND policyname = 'Users can create team members for workspaces they own') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'webhooks' AND policyname = 'Users can manage their own webhooks') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'training_datasets' AND policyname = 'Users can manage their own training datasets') THEN
    ALTER TABLE public.training_datasets ENABLE ROW LEVEL SECURITY;
    CREATE POLICY "Users can manage their own training datasets" ON public.training_datasets FOR ALL USING (auth.uid() = user_id);
  END IF;
END $$;

DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'custom_models' AND policyname = 'Users can manage their own custom models') THEN
    ALTER TABLE public.custom_models ENABLE ROW LEVEL SECURITY;
    CREATE POLICY "Users can manage their own custom models" ON public.custom_models FOR ALL USING (auth.uid() = user_id);
  END IF;
END $$;

DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'mobile_settings' AND policyname = 'Users can manage their own mobile settings') THEN
    ALTER TABLE public.mobile_settings ENABLE ROW LEVEL SECURITY;
    CREATE POLICY "Users can manage their own mobile settings" ON public.mobile_settings FOR ALL USING (auth.uid() = user_id);
  END IF;
END $$;
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.triggers WHERE trigger_name = 'update_team_workspaces_updated_at') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.triggers WHERE trigger_name = 'update_webhooks_updated_at') THEN
    CREATE TRIGGER update_webhooks_updated_at
      BEFORE UPDATE ON public.webhooks
      FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.triggers WHERE trigger_name = 'update_training_datasets_updated_at') THEN
    CREATE TRIGGER update_training_datasets_updated_at
      BEFORE UPDATE ON public.training_datasets
      FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.triggers WHERE trigger_name = 'update_custom_models_updated_at') THEN
    CREATE TRIGGER update_custom_models_updated_at
      BEFORE UPDATE ON public.custom_models
      FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.triggers WHERE trigger_name = 'update_mobile_settings_updated_at') THEN
