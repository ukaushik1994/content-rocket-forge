  ...
  <div className="space-y-4">
    <div className="flex gap-2">
      {[
        ...
        <span key={p.name} className={`px-3 py-1.5 rounded-full text-xs font-medium border ${p.active ? 'bg-neon-blue/15 border-neon-blue/30 text-neon-blue' : 'bg-white/[0.03] border-white/[0.08] text-muted-foreground'}`}>
          {p.name}
        </span>
      ))}
    </div>
