...
const ContentGapsInner: React.FC = () => {
  const { currentStrategy, insights } = useContentStrategy();

  const serpMetrics = React.useMemo(() =>
    insights.find(insight => insight.keyword === currentStrategy?.main_keyword)?.serp_data || null,
  ...
  return (
    <section aria-labelledby="content-gaps-heading">
      <h2 id="content-gaps-heading" className="sr-only">Content Gaps Module</h2>
      <ContentGapsTab serpMetrics={serpMetrics} goals={goals} />
    </section>
  );
};

const ContentGapsPage: React.FC = () => {
  const canonicalUrl = typeof window !== 'undefined'
    ? `${window.location.origin}/research/content-gaps`
  ...
  return (
    <ContentStrategyProvider>
      <div className="min-h-screen bg-background relative overflow-hidden">
        <Helmet>
          <title>Content Gaps — Competitive Opportunity Analysis</title>
          <meta name="description" content="Discover content gaps from SERP intelligence. Analyze, select, and action opportunities seamlessly." />
          ...
          >
            ...
            <div className="relative">
              ...
              >
                ...
                <span className="text-sm font-medium">AI-Powered Gap Finder</span>
              ...
              <p className="text-lg text-muted-foreground max-w-2xl mx-auto mb-8">
                Identify and action competitive gaps with signals from the SERP.
              </p>

              {/* Quick stats */}
              <div className="flex justify-center gap-6">
                {[{Icon: FileSearch, label: 'Analyze'}, {Icon: BarChart3, label: 'Signals'}, {Icon: Zap, label: 'Real-time'}].map(({Icon, label}) => (
...
export default ContentGapsPage;