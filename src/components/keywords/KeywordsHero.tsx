...
export const KeywordsHero: React.FC<KeywordsHeroProps> = ({
  ...
  const quickFilters = [
    { key: 'all', label: 'All Keywords', count: keywordStats.total, icon: Database, color: 'bg-blue-500/20 text-blue-400' },
    { key: 'published', label: 'Published', count: keywordStats.inPublished, icon: FileText, color: 'bg-green-500/20 text-green-400' },
    { key: 'draft', label: 'Draft', count: keywordStats.inDraft, icon: BarChart3, color: 'bg-yellow-500/20 text-yellow-400' },
    { key: 'cannibalization', label: 'Warnings', count: keywordStats.cannibalization, icon: AlertTriangle, color: 'bg-orange-500/20 text-orange-400' },
  ];
    ...
    >
      ...
      >
        <div className="flex items-center gap-2 px-6 py-3 bg-background/80 backdrop-blur-xl rounded-full border border-border/50">
          <div className={`w-2 h-2 rounded-full transition-all duration-500 ${keywordStats.total > 0 ? 'bg-primary' : 'bg-primary/30'}`} />
          <div className={`w-2 h-2 rounded-full transition-all duration-500 ${keywordStats.inPublished > 0 ? 'bg-primary' : 'bg-primary/30'}`} />
          <div className={`w-2 h-2 rounded-full transition-all duration-500 ${keywordStats.cannibalization === 0 && keywordStats.total > 0 ? 'bg-primary' : 'bg-primary/30'}`} />
          <span className="text-xs font-medium text-muted-foreground ml-2">
            {keywordStats.total === 0 ? 'Getting Started' : keywordStats.cannibalization > 0 ? 'Needs Review' : 'Optimized'}
          </span>
        </div>
      </motion.div>
      ...
      <div className="relative z-10 w-full px-6 pt-24 pb-12">
        ...
        >
          ...
          <div className="relative">
            ...
            >
              {[
                { icon: TrendingUp, label: "Total Keywords", value: keywordStats.total },
                { icon: BarChart3, label: "In Published", value: keywordStats.inPublished },
                { icon: Zap, label: "Warnings", value: keywordStats.cannibalization }
              ].map((stat, index) => (
                <motion.div 
                  key={stat.label}
