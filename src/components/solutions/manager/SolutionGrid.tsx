  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {solutions.map(solution => (
        <div key={solution.id} className="relative group transition-all duration-200 hover:translate-y-[-4px]">
          <SolutionCard 
            name={solution.name}
          ...
          <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity flex space-x-1">
            <Button 
              variant="outline" 
              size="sm" 
              className="h-7 bg-black/50 backdrop-blur-sm hover:bg-black/70"
              onClick={(e) => {
                e.stopPropagation();
                onEdit(solution);
              }}
            ...
            <Button 
              variant="destructive" 
              size="sm" 
              className="h-7 bg-red-500/80 backdrop-blur-sm hover:bg-red-500/90"
              onClick={(e) => {
                e.stopPropagation();
                onDelete(solution);
              }}
