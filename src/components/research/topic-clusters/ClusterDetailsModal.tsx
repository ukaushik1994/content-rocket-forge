...
export function ClusterDetailsModal({ 
  ...
  const [contentOpportunities, setContentOpportunities] = useState<string[]>([]);

  useEffect(() => {
    if (clusterId && isOpen) {
      const clusterData = topicClusterService.getCluster(clusterId);
      setCluster(clusterData);
      
      if (clusterData) {
        const opportunities = topicClusterService.generateContentOpportunities(clusterData);
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <Tabs defaultValue="overview" className="space-y-4">
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-semibold">{cluster.completion}%</span>
                  </div>
                  ...
                  {cluster.targetAudience && (
                    <div>
                      ...
                      <p className="text-muted-foreground">{cluster.targetAudience}</p>
                    </div>
                  )}
                </div>
              </CardContent>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <Card>
                <CardContent className="p-4 text-center">
                  ...
                  <div className="text-2xl font-bold">{cluster.articles}</div>
              ...
              <Card>
                <CardContent className="p-4 text-center">
                  ...
                  <div className="text-2xl font-bold">{formatTraffic(cluster.totalTraffic)}</div>
              ...
              <Card>
                <CardContent className="p-4 text-center">
                  ...
                  <div className="text-2xl font-bold">{cluster.avgPosition.toFixed(1)}</div>
          ...
          <TabsContent value="keywords" className="space-y-4">
            <Card>
              <CardHeader>
                <CardTitle>Related Keywords ({cluster.keywords.length})</CardTitle>
              </CardHeader>
              <CardContent>
              ...
              <CardContent>
                <div className="space-y-6">
                  <div className="grid grid-cols-2 gap-6">
                    <div>
                      ...
                      <div className="text-2xl font-bold text-green-500">
                        {formatTraffic(cluster.totalTraffic)}
                      </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between items-center">
                      <span className="text-sm">Keywords Covered</span>
                      <span className="font-medium">{cluster.keywords.length}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm">Last Updated</span>
                      <span className="font-medium">{cluster.lastUpdated}</span>
                    </div>
                    <div className="flex justify-between items-center">
