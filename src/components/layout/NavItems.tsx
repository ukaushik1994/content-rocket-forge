...
const NavItem: React.FC<NavItemProps> = ({
  ...
  label,
  active
}) => <Link to={to} className={cn('flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-colors', active ? 'bg-primary text-primary-foreground' : 'hover:bg-muted text-muted-foreground hover:text-foreground')}>
    {icon}
    {label}
  </Link>;
