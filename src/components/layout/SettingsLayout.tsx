...
import { CreditCard, Key, User, Bell, Palette, Download, Settings as SettingsIcon, FileText } from 'lucide-react';
  ...
  return <div className="flex w-full min-h-screen">
      ...
      <aside className="w-80 border-r border-white/10 bg-gradient-to-b from-background/95 via-background/90 to-neon-purple/5 backdrop-blur-xl flex-shrink-0">
        <div className="p-6 h-full">
          ...
          <Tabs value={activeTab} onValueChange={handleValueChange} orientation="vertical" className="h-full">
            <TabsList className="flex flex-col h-auto w-full bg-transparent space-y-2 p-0">
              {tabs.map((tab, index) => <motion.div key={tab.value} initial={{
              opacity: 0,
              x: -30
            }} animate={{
              opacity: 1,
