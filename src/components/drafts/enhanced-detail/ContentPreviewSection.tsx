...
import { Copy, Download, BookOpen, Clock, Eye, Hash, BarChart3, CheckCircle2 } from 'lucide-react';
import { motion } from 'framer-motion';
import { toast } from 'sonner';
import { ErrorBoundary } from '@/components/common/ErrorBoundary';
import { LoadingSkeleton, ContentPreviewSkeleton } from '@/components/common/LoadingSkeleton';
import { calculateContentQuality, calculateReadabilityMetrics, getContentInsights } from '@/utils/contentAnalytics';
import { generateHeadingId } from '@/utils/scrollUtils';
...
export const ContentPreviewSection: React.FC<ContentPreviewSectionProps> = ({
  ...
  // Enhanced content statistics with analytics
  const contentStats = useMemo(() => {
    ...
    const quality = calculateContentQuality(content, keywords, headings.length > 0);
    const readability = calculateReadabilityMetrics(content);
    const insights = getContentInsights(content, keywords);
  ...
  const scrollToHeading = useCallback((headingText: string, headingId: string) => {
  ...
  return (
    <ErrorBoundary fallbackTitle="Content Preview Error" onRetry={() => window.location.reload()}>
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 h-full">
        ...
        <div className="lg:col-span-1 space-y-4">
          ...
          <Card className="bg-gradient-to-br from-primary/5 to-primary/10 border-primary/20">
            ...
            <CardContent className="space-y-4">
              ...
              {/* Quality Score */}
              {contentStats.quality && (
                <div className="pt-2 border-t border-white/10">
                  <div className="flex items-center justify-between text-sm mb-2">
                    <span className="text-muted-foreground">Quality Score:</span>
                    <span className="font-bold text-primary">{contentStats.quality.overallScore}%</span>
                  </div>
                  <div className="w-full bg-muted/30 rounded-full h-2">
                    <div 
                      className={`h-2 rounded-full transition-all duration-500 ${
                        contentStats.quality.overallScore >= 80 ? 'bg-green-500' :
                        contentStats.quality.overallScore >= 60 ? 'bg-yellow-500' : 'bg-red-500'
                      }`}
                      style={{ width: `${contentStats.quality.overallScore}%` }}
                    />
                  </div>
                </div>
              )}
                ...
                <ScrollArea className="h-48">
                  <div className="space-y-1">
                    {contentStats.headings.map((heading, index) => (
                      <motion.button
                        key={index}
                        onClick={() => scrollToHeading(heading.text, heading.id)}
          ...
          <Card className="h-full bg-gradient-to-br from-background via-background/95 to-background/90 border border-white/10">
            ...
            <CardContent className="p-0 h-full">
              <ScrollArea className="h-[calc(100%-4rem)] p-6">
                <div className="prose prose-sm dark:prose-invert max-w-none content-container" ref={contentRef}>
                  {content ? (
                    <motion.div 
                        ...
                        if (paragraph.match(/^#{1,6}\s+/)) {
                          ...
                          return (
                            <HeadingTag
                              key={index}
                              id={headingId}
                              className={`font-bold mt-8 mb-4 text-foreground scroll-mt-6 ${
                                level === 1 ? 'text-2xl' : 
                                level === 2 ? 'text-xl' : 
