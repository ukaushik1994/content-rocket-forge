...
const RepurposedContentDialog: React.FC<RepurposedContentDialogProps> = ({
  ...
  const relatedFormats = availableFormats.filter(f => generatedFormats.includes(f.id) || f.id === content.formatId).map(f => ({
    id: f.id,
    name: f.name,
  ...
  return <Dialog open={open} onOpenChange={open => !open && onClose()}>
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-hidden flex flex-col bg-gradient-to-b from-black/95 to-black/90 border border-white/20 shadow-xl shadow-indigo-500/10 p-0 rounded-xl backdrop-blur-lg">
        ...
        {relatedFormats.length > 1 && <motion.div initial={{
        opacity: 0,
        y: -5
      }} animate={{
        opacity: 1,
        y: 0
            ...
            <div className="flex items-center gap-2 overflow-x-auto scrollbar-none">
              {relatedFormats.map(relatedFormat => <FormatButton key={relatedFormat.id} formatId={relatedFormat.id} name={relatedFormat.name} isActive={relatedFormat.isActive} onClick={() => {}} className={relatedFormat.isActive ? "bg-indigo-500/90 text-white shadow-md shadow-indigo-500/20" : "bg-white/5 hover:bg-white/10 transition-colors duration-200"} />)}
            </div>
          </motion.div>}
        ...
        delay: 0.1
          ...
          <DialogActionButtons onCopy={() => onCopy(content.content)} onDownload={() => onDownload(content.content, formatName)} onDelete={onDelete ? handleDelete : undefined} isDeleting={isDeleting} isSaving={isSaving} />
        </motion.div>
      </DialogContent>
    </Dialog>;
};
