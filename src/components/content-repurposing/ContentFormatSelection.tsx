        ...
        <div className="space-y-3">
          {contentFormats.map(format => (
            <div 
              ...
              className={`p-3 border roundeI'll help you fix the TypeScript error in ContentRepurposing.tsx. The issue is a type mismatch in the function signature for the `handleGenerateContent` prop being passed to the `ContentFormatSelection` component.

Let's examine what's happening:

1. The `ContentFormatSelection` component expects `onGenerateContent` to be a function that takes no arguments `() => void`
2. However, the `handleGenerateContent` function from `useContentRepurposing` has the signature `(contentTypeIds: string[]) => Promise<void>`
3. We need to align these function signatures

<lov-code>
Let's fix the error in the ContentRepurposing.tsx and ContentFormatSelection.tsx files:

First, let's update the ContentFormatSelection.tsx component to accept a function that takes string array as parameter:

<lov-write file_path="src/components/content-repurposing/ContentFormatSelection.tsx">
import React from 'react';
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from '@/components/ui/card';
import { Badge } from '@/components/ui/badge';
import { Button } from '@/components/ui/button';
