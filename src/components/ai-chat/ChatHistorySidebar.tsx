...
import { MessageSquare, Search, Plus, MoreVertical, Trash2, X, History, Settings, ChevronDown, Loader2, Pin, Archive, Filter, SortAsc, Tag } from 'lucide-react';
...
export const ChatHistorySidebar: React.FC<ChatHistorySidebarProps> = ({
  ...
  return <motion.div className={`fixed left-0 top-16 bottom-0 w-80 bg-background/80 backdrop-blur-xl border-r border-border/50 flex flex-col z-40 ${className}`} variants={sidebarVariants} initial="hidden" animate="visible" exit="hidden">
      {/* Header */}
      <div className="p-4 border-b border-white/10">
        <div className="flex items-center justify-between mb-4">
          ...
          <Button variant="ghost" size="sm" onClick={onToggleSidebar} className="text-white/60 hover:text-white hover:bg-white/10">
            <X className="h-4 w-4" />
          </Button>
        </div>

        {/* New Chat Button */}
        <Button onClick={onCreateConversation} className="w-full bg-gradient-to-r from-purple-500/20 to-blue-500/20 hover:from-purple-500/30 hover:to-blue-500/30 border border-white/20 text-white">
      ...
      <div className="p-4 border-b border-white/10">
        <div className="relative">
          ...
          <Input placeholder="Search conversations..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10 bg-white/5 border-white/20 text-white placeholder:text-white/40 focus:border-purple-500/50" />
        </div>
        ...
        <div className="p-2">
          <AnimatePresence mode="wait">
            {conversations && conversations.length > 0 ? filteredConversations.length > 0 ? <>
                  {displayedConversations.map((conversation, index) => <motion.div key={conversation.id} custom={index} variants={itemVariants} initial="hidden" animate="visible" exit="hidden" whileHover={{
              scale: 1.02
            }} whileTap={{
              scale: 0.98
            }}>
                    <Card className={`mb-2 p-3 cursor-pointer transition-all duration-200 group border-white/10 hover:border-purple-500/30 ${activeConversation === conversation.id ? 'bg-gradient-to-r from-purple-500/20 to-blue-500/20 border-purple-500/50' : 'bg-white/5 hover:bg-white/10'}`} onClick={() => onSelectConversation(conversation.id)}>
                      <div className="flex items-start justify-between">
                         <div className="flex-1 min-w-0">
                          ...
                          addSuffix: true
                             ...
                             {conversation.tags && conversation.tags.length > 0 && <Badge variant="secondary" className="bg-white/10 text-white/70 text-xs h-5">
                        ...
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <Button variant="ghost" size="sm" className="opacity-0 group-hover:opacity-100 h-8 w-8 p-0 text-white/60 hover:text-white hover:bg-white/20" onClick={e => e.stopPropagation()}>
                        ...
                        onPinConversation?.(conversation.id);
                            ...
                            {onArchiveConversation && <DropdownMenuItem onClick={e => {
                        e.stopPropagation();
                  ...
                  {/* Load More Button */}
                  {hasMoreConversations && <motion.div initial={{
              opacity: 0,
              y: 20
            }} animate={{
              opacity: 1,
              y: 0
                      ...
                      <Button onClick={handleLoadMore} disabled={isLoadingMore} variant="ghost" className="w-full text-white/60 hover:text-white hover:bg-white/10 border border-white/20 hover:border-white/30">
                        {isLoadingMore ? <>
      ...
      <div className="p-4 border-t border-white/10">
        <div className="flex items-center justify-between text-xs text-white/50 mb-3">
          <span>
            {filteredConversations.length > 0 ? `${Math.min(displayLimit, filteredConversations.length)} of ${filteredConversations.length} shown` : `${conversations.length} conversations`}
          </span>
