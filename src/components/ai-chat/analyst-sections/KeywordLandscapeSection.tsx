...
export const KeywordLandscapeSection: React.FC<Props> = ({ topics, platformData, onSendMessage }) => {
  ...
  return (
    <AnalystSectionWrapper number="05" label="Keyword Landscape" headline={<>Your keyword presence is <span className="text-blue-400">evolving</span></>} delay={0.2}>
      ...
      {keywordTopics.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          {keywordTopics.map((topic) => (
            ...
            >
              {topic.name} {topic.mentionCount > 1 && <span className="text-primary/60">×{topic.mentionCount}</span>}
            </button>
          ))}
        </div>
      )}
