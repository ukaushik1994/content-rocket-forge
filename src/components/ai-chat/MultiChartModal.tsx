...
// Helper functions for SERP-style static Tailwind classes
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 mb-1">
        {Icon && <Icon className={`h-4 w-4 ${getIconColor(colorTheme)}`} />}
      ...
      {(trend || change) && (
        <div className="flex items-center gap-1 mt-2 text-xs text-muted-foreground">
          ...
          <span className={trendUp ? 'text-green-400' : 'text-red-400'}>
            {trend || (change ? `${change.value > 0 ? '+' : ''}${change.value}%` : '')}
          </span>
          {change?.period && (
            <span className="text-muted-foreground/70">{change.period}</span>
          )}
        </div>
      )}
    </motion.div>
...
const KeyMetricsPanel: React.FC<{ visualData?: VisualData; context?: any }> = ({ visualData, context }) => {
  ...
  return (
    <div className={`grid ${getGridClass(metrics.length)} gap-4 mb-6`}>
