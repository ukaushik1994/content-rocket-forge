    ...
    <div className="space-y-6">
      ...
      {selectedItems.length > 0 && (
        <Card className="border border-white/10">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm">Selected from SERP Analysis</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
              <p className="text-xs text-muted-foreground">
                The AI will use these {selectedItems.length} selected items to generate your content outline:
              </p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {['keyword', 'question', 'entity', 'heading', 'contentGap', 'topRank'].map(type => {
                  const typeItems = selectedItems.filter(item => item.type === type);
                  if (typeItems.length === 0) return null;
                  
                  let typeLabel = '';
                  switch(type) {
                    case 'keyword': typeLabel = 'Keywords'; break;
                    case 'question': typeLabel = 'Questions'; break;
                    case 'entity': typeLabel = 'Entities'; break;
                    case 'heading': typeLabel = 'Headings'; break;
                    case 'contentGap': typeLabel = 'Content Gaps'; break;
                    case 'topRank': typeLabel = 'Top Ranks'; break;
                    default: typeLabel = type;
                  }
                  
                  return (
                    <Card key={type} className="bg-white/5 border border-white/10">
                      <CardContent className="p-3">
                        <p className="text-xs font-medium mb-2">{typeLabel} ({typeItems.length})</p>
