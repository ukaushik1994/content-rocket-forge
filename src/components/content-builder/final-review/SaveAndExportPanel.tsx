...
export const SaveAndExportPanel: React.FC<SaveAndExportPanelProps> = ({
  ...
  const handleSave = async () => {
    try {
      ...
      if (user?.user) {
        const {
          data: contentItem
        } = await supabase.from('content_items').select('id').eq('user_id', user.user.id).eq('status', 'draft').order('created_at', {
          ascending: false
        }).limit(1).single();
        if (contentItem) {
  ...
  return <>
      <Card className={cn("border p-4", isReady ? "border-green-500/30 bg-gradient-to-br from-green-950/20 to-black/20" : "border-amber-500/30 bg-gradient-to-br from-amber-950/20 to-black/20")}>
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div className="flex items-center gap-3">
            <div className={cn("p-2 rounded-full", isReady ? "bg-green-500/20" : "bg-amber-500/20")}>
              {isReady ? <CheckCircle className="h-5 w-5 text-green-400" /> : <AlertTriangle className="h-5 w-5 text-amber-400" />}
            </div>
            <div>
              <h3 className="font-medium">{isReady ? "Ready to Save & Export" : "Content Status"}</h3>
              <p className="text-sm text-muted-foreground">
                {isReady ? "Your content has passed all checks" : `Content is ${completionPercentage}% optimized, needs improvement`}
              </p>
              <div className="flex items-center gap-2 mt-1">
                <Badge variant="outline" className={cn(isReady ? "border-green-500/30 text-green-500" : isOkay ? "border-amber-500/30 text-amber-500" : "border-red-500/30 text-red-500")}>
                  {isReady ? "Ready for publishing" : isOkay ? "Needs minor improvements" : "Needs major improvements"}
                </Badge>
                {isSavedToDraft && <Badge variant="outline" className="border-blue-500/30 text-blue-500">
          ...
          <div className="flex items-center gap-2 self-end sm:self-auto">
            <Button variant="outline" onClick={handleSave} disabled={isSaving} className="border-white/10 hover:bg-white/5">
              {isSaving ? <>
            ...
            <Button onClick={handlePublish} disabled={isSaving} className={cn("gap-2", isReady ? "bg-gradient-to-r from-green-600 to-green-500 hover:from-green-700 hover:to-green-600" : "bg-gradient-to-r from-purple-600 to-violet-500 hover:from-purple-700 hover:to-violet-600")}>
