    ...
    <div className="mb-4 p-3 border border-dashed rounded-md bg-secondary/10">
      ...
      <div className="flex flex-wrap gap-1.5 mb-3">
        ...
        {selectedKeywords.map((keyword, idx) => (
          <Badge key={`keyword-${idx}`} className="bg-blue-500/70 hover:bg-blue-500">
            {keyword}
          </Badge>
        ))}
        {selectedKeywordItems.map((item, idx) => (
          <Badge key={`serp-keyword-${idx}`} className="bg-blue-400/60 hover:bg-blue-400">
            {item.content}
          </Badge>
        ))}
      </div>
      ...
      {(selectedQuestions.length > 0 || selectedHeadings.length > 0 || selectedEntities.length > 0 || selectedContentGaps.length > 0 || selectedTopRanks.length > 0) && (
        <>
          ...
          <Button variant="outline" size="sm" className="mt-3 w-full text-xs" onClick={onIncorporateAllSerp}>
            ...
            Select All SERP Items for AI Integration
          </Button>
        </>
      )}
    </div>
