...
import { TrendingUp, Target, Lightbulb, ArrowRight, Calendar, FileText, Book, Zap, AlertCircle, CheckCircle, Info, ExternalLink } from 'lucide-react';
...
export const DashboardSummary = () => {
  ...
  const fetchDashboardSummary = async () => {
    try {
      ...
      const [dashboardResponse, opportunitiesData, aiStrategiesData] = await Promise.all([supabase.functions.invoke('dashboard-summary'), opportunityHunterService.getOpportunities().catch(() => []), aiStrategyService.getStrategies().catch(() => [])]);
  ...
  const combinedOpportunities = React.useMemo(() => {
    ...
    aiStrategies.forEach(strategy => {
      strategy.proposals?.forEach((proposal: any) => {
        ...
        const normalizedKeywords = proposal.keywords ? proposal.keywords.map((k: any) => typeof k === 'string' ? k : k.keyword || String(k)) : [];
        combined.push({
          ...proposal,
  ...
  return <div className="space-y-8 relative">
      ...
      <div className="absolute inset-0 -z-10">
        <motion.div className="absolute top-[20%] left-[10%] w-[400px] h-[400px] rounded-full bg-gradient-to-r from-neon-purple/20 via-neon-blue/15 to-transparent blur-[100px]" animate={{
        x: [0, 30, 0],
        y: [0, -20, 0],
        scale: [1, 1.1, 1]
      }} transition={{
        ...
        <motion.div className="absolute bottom-[10%] right-[15%] w-[300px] h-[300px] rounded-full bg-gradient-to-l from-neon-pink/15 via-neon-purple/10 to-transparent blur-[80px]" animate={{
        x: [0, -25, 0],
        y: [0, 15, 0],
        scale: [1, 0.9, 1]
      }} transition={{
      ...
      delay: 0.2
        ...
        isPercentage: true
      }].map((item, index) => <motion.div key={item.type} whileHover={{
        scale: 1.02,
        y: -2
      }} transition={{
        type: "spring",
            ...
            <Card className="glass-panel border-white/10 bg-white/5 backdrop-blur-xl hover:bg-white/10 transition-all duration-300 group overflow-hidden relative">
              ...
              <CardContent className="p-6 relative z-10">
                ...
                <div className="space-y-2">
                  <Progress value={item.isPercentage ? item.current : item.current / Math.max(item.goal, 1) * 100} className="h-2 bg-white/20" />
      ...
      {/* Top Performer */}
      {data.top_performer && <motion.div initial={{
      opacity: 0,
      x: -30
    }} animate={{
      opacity: 1,
              ...
              <div className="space-y-4">
                <div className="p-4 rounded-lg bg-white/5 border border-white/10">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    {data.top_performer.growth > 0 && <Badge className="bg-gradient-to-r from-emerald-500/20 to-green-500/20 text-emerald-400 border-emerald-500/30">
                        +{data.top_performer.growth}%
                      </Badge>}
                  </div>
                </div>
                <div className="flex gap-3">
                  <Button variant="outline" size="sm" asChild className="border-white/20 bg-white/10 hover:bg-white/20 text-white hover-scale">
                    <a href={data.top_performer.url}>
                      View Content
                      <ExternalLink className="w-3 h-3 ml-1" />
                    </a>
                  </Button>
                  <Button variant="outline" size="sm" className="border-neon-purple/30 bg-neon-purple/10 hover:bg-neon-purple/20 text-neon-purple hover-scale">
                    Repurpose
                  </Button>
                </div>
              </div>
      ...
      delay: 0.6
        ...
        <Card className="glass-panel border-white/10 bg-white/5 backdrop-blur-xl hover:bg-white/10 transition-all duration-300 group overflow-hidden relative">
          ...
          <motion.div className="absolute inset-0 bg-gradient-to-br from-neon-pink/10 via-neon-purple/10 to-neon-blue/10 opacity-0 group-hover:opacity-100 transition-opacity duration-500" animate={{
          background: ["linear-gradient(135deg, rgba(255,20,147,0.1), rgba(138,43,226,0.1), rgba(30,144,255,0.1))", "linear-gradient(135deg, rgba(30,144,255,0.1), rgba(255,20,147,0.1), rgba(138,43,226,0.1))", "linear-gradient(135deg, rgba(138,43,226,0.1), rgba(30,144,255,0.1), rgba(255,20,147,0.1))"]
        }} transition={{
          duration: 8,
          repeat: Infinity,
          ease: "easeInOut"
          ...
          <CardHeader className="relative z-10">
            ...
            <CardDescription className="text-white/60 ml-13">
              {combinedOpportunities.length > 0 ? `${combinedOpportunities.length} content opportunities available` : "No opportunities available"}
            </CardDescription>
          </CardHeader>

          <CardContent className="relative z-10 p-0">
            {combinedOpportunities.length > 0 ? <div className="relative overflow-hidden">
                {/* Carousel Container */}
                <motion.div className="flex gap-6 px-6 pb-6" animate={{
              x: [0, -50, 0]
            }} transition={{
              duration: 20,
              repeat: Infinity,
              ...
              width: `${Math.max(combinedOpportunities.length * 320, typeof window !== 'undefined' ? window.innerWidth : 1200)}px`
                  ...
                  {combinedOpportunities.map((item, index) => <motion.div key={`${item.type}-${index}`} className="flex-shrink-0 w-80 p-4 rounded-lg bg-white/5 border border-white/10 hover:bg-white/10 transition-all duration-300 cursor-pointer group/tile" whileHover={{
                scale: 1.02,
                y: -2
                      ...
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex items-center gap-2">
                          <div className={`w-6 h-6 rounded-full flex items-center justify-center ${item.type === 'opportunity' ? 'bg-neon-blue/20 text-neon-blue' : item.type === 'ai-strategy' ? 'bg-neon-purple/20 text-neon-purple' : 'bg-neon-pink/20 text-neon-pink'}`}>
                            {getOpportunityTypeIcon(item.type)}
                          </div>
                          <Badge variant="outline" className="text-xs border-white/20 text-white/70">
                            {item.type === 'opportunity' ? 'Opportunity' : item.type === 'ai-strategy' ? 'AI Strategy' : 'Dashboard'}
                          </Badge>
                        </div>
                        {item.priority && <Badge className={`text-xs ${getPriorityColor(item.priority)} bg-white/10 border-white/20`}>
                            {item.priority}
                          </Badge>}
                      </div>
                      ...
                      {/* Description */}
                      {item.description && <p className="text-sm text-white/60 mb-3 line-clamp-3 leading-relaxed">
                          {item.description}
                        </p>}

                      {/* Keywords */}
                      {item.keywords?.length > 0 && <div className="mb-3">
                          <div className="flex flex-wrap gap-1">
                            {item.keywords.slice(0, 3).map((keyword: string, idx: number) => <Badge key={idx} variant="outline" className="text-xs border-white/10 text-white/50 bg-white/5">
                                {keyword}
                              </Badge>)}
                            {item.keywords.length > 3 && <Badge variant="outline" className="text-xs border-white/10 text-white/40 bg-white/5">
                      ...
                      <div className="flex items-center justify-between text-xs text-white/50 mb-4">
                        <div className="flex items-center gap-3">
                          {item.volume && <span className="flex items-center gap-1">
                              <TrendingUp className="w-3 h-3" />
                              {item.volume > 1000 ? `${(item.volume / 1000).toFixed(1)}k` : item.volume}
                            </span>}
                          {item.difficulty !== undefined && <span className="flex items-center gap-1">
                              <Target className="w-3 h-3" />
                              {item.difficulty}/100
                            </span>}
                          {item.content_format && <span className="capitalize">{item.content_format}</span>}
                        </div>
                      ...
                      {/* Action Button */}
                      <Button size="sm" className="w-full bg-white/10 hover:bg-white/20 text-white border-white/20 group-hover/tile:bg-neon-blue/20 group-hover/tile:border-neon-blue/30 transition-all duration-300">
        ...
        <Card className="glass-panel border-white/10 bg-white/5 backdrop-blur-xl text-center overflow-hidden relative">
          ...
          <CardContent className="p-8 relative z-10">
            ...
            <p className="text-white/60 mb-6">
              You're making great progress on your content goals for {new Date(data.month).toLocaleDateString('en-US', {
              month: 'long',
              year: 'numeric'
            })}
            </p>
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <Button className="bg-gradient-to-r from-neon-blue/20 to-neon-purple/20 hover:from-neon-blue/30 hover:to-neon-purple/30 text-white border border-white/20 hover-scale" onClick={() => navigate('/content-builder')}>
                <FileText className="w-4 h-4 mr-2" />
                Create New Content
              </Button>
              <Button variant="outline" className="border-white/20 bg-white/10 hover:bg-white/20 text-white hover-scale" onClick={() => navigate('/research/content-strategy')}>
