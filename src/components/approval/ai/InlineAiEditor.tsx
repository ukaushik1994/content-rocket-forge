...
export const InlineAiEditor: React.FC<InlineAiEditorProps> = ({
  ...
  const runInlineAi = useCallback(async (action: 'rephrase' | 'shorten' | 'expand' | 'fix') => {
    ...
    try {
      ...
      if (userInstruction && userInstruction.trim()) {
        const {
          saveUserInstruction
        } = await import('@/services/userInstructionsService');
        await saveUserInstruction(userInstruction, 'inline_editing', undefined,
        // Format type not applicable for inline editing
        undefined,
        // No specific content ID for inline editing
        undefined // No session ID for inline editing
        );
      }
      const result = await AIServiceController.generate('content_generation', system, user, {
        maxTokens: 400,
        temperature: 0.2
      });
  ...
  const revertLastEdit = useCallback(() => {
    ...
    setSelection({
      start,
      end: start + prevText.length
    });
    setLastEdit(null);
  }, [lastEdit, value, onChange]);
  return <div ref={containerRef} className={`relative h-full flex flex-col ${className || ''}`}>
      {selection && toolbarPos && <div ref={toolbarRef} className="absolute z-10 flex flex-wrap items-center gap-1 glass-card rounded-lg px-2 py-1 max-w-[92vw] shadow-xl ring-1 ring-white/10 animate-enter" style={{
      left: toolbarPos.left,
      top: toolbarPos.top,
      transform: toolbarPos.placement === 'above' ? 'translate(-50%, calc(-100% - 8px))' : 'translate(-50%, 8px)'
    }}>
          <Input value={userInstruction} onChange={e => setUserInstruction(e.target.value)} placeholder="Add instruction (optional)" aria-label="Custom AI instruction" className="h-8 w-44 text-xs" disabled={isProcessing || disabled} />
