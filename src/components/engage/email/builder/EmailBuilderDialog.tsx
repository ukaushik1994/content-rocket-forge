...
import { Undo2, Redo2, Monitor, Smartphone, Save, Eye, Paintbrush, Code, X } from 'lucide-react';
...
export const EmailBuilderDialog: React.FC<EmailBuilderDialogProps> = ({
  open, onOpenChange, initialBlocks, onSave, templateName,
}) => {
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="flex items-center justify-between px-4 py-2 border-b border-border/50 bg-card/95 backdrop-blur-xl shrink-0">
            <div className="flex items-center gap-3">
              <button onClick={() => onOpenChange(false)} className="p-1.5 hover:bg-muted/50 rounded-md transition-colors">
                <X className="h-4 w-4 text-muted-foreground" />
              </button>
              <div className="h-5 w-px bg-border/50" />
              <span className="text-sm font-medium text-foreground">{templateName || 'Email Builder'}</span>
            </div>
