...
import { CampaignConversationMessage } from '@/hooks/useCampaignConversation';
import { User, Bot, Copy, Check } from 'lucide-react';
import { formatDistanceToNow } from 'date-fns';
import { useToast } from '@/hooks/use-toast';

interface CampaignMessageBubbleProps {
  message: CampaignConversationMessage;
  isLatest: boolean;
}

export const CampaignMessageBubble: React.FC<CampaignMessageBubbleProps> = ({
  message,
  isLatest
}) => {
  ...
  console.log('[CampaignMessageBubble] Rendering:', message.role, message.content.substring(0, 50));
  ...
  return (
    ...
    >
      ...
      {isUser && (
        <motion.div 
          className="flex h-8 w-8 items-center justify-center rounded-full bg-gradient-to-br from-primary to-primary/80 border border-primary/30 flex-shrink-0"
          initial={{ scale: 0 }}
          animate={{ scale: 1 }}
          transition={{ delay: 0.1, type: "spring", stiffness: 300 }}
        >
